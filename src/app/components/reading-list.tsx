...
export default function ReadingList (): JSX.Element | null {
  ...
  return (
    <div className="flex flex-row fixed z-50 max-h-[100vh]">
      <main className="flex flex-col gap-2 bg-blue-200 p-2 rounded-r-sm h-screen overflow-scroll w-fit ease-in-out transition-all duration-300"
        ...
        {bookmarks.reverse().map((isbn: string) => {
          ...
          return (
            <div key={isbn} className="relative [&>button]:hidden [&>button]:hover:flex group">
              <Button
                className="absolute brightness-100 top-[70%] left-[30%] text-white py-1 px-0 w-fit h-fit rounded z-10 group-hover:brightness-80 bg-[#ff0000]"
                isIconOnly
